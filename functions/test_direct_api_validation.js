/**
 * Direct AI API Validation Script
 * Tests DeepSeek and Gemini APIs directly with real concept cards
...
function buildConceptContext(card) {
  ...
  return {
    ...
    topicTitle: card.topic?.title || 'this concept'
  };
}
...
KEY CONCEPTS: ${Object.keys(conceptContext.vocabulary).join(', ')}
 ...
 */
async function testGemini(question, conceptContext, cardInfo) {
  const startTime = Date.now();
  
  try {
    console.log(`\n🧠 Testing GEMINI: "${question}"`);
 ...
 */
async function testDeepSeek(question, conceptContext, cardInfo) {
  const startTime = Date.now();
  
  try {
    console.log(`\n🤖 Testing DEEPSEEK: "${question}"`);
    console.log(`   📚 Card: ${cardInfo.id} (${cardInfo.subject} - ${cardInfo.level})`);
    ...
    // Analyze response quality
    const analysis = analyzeResponse(content, question, conceptContext);
...
function analyzeResponse(response, question, conceptContext) {
  ...
  const encouragingWords = ['great', 'excellent', 'wonderful', 'good', 'nice', 'well done', 'exactly', 'perfect', 'fantastic', 'awesome'];
  ...
  const hasMath = /\d+°|\d+x|\d+=|\d+\+|\d+-|\d+×|\d+÷|\d+\^/.test(response) || 
                 lowerResponse.includes('equation') || 
                 lowerResponse.includes('formula') ||
                 lowerResponse.includes('angle') ||
                 lowerResponse.includes('factor') ||
                 lowerResponse.includes('solve');
  if (hasMath) {
  ...
  const relevantTerms = [
    ...
    conceptContext.topicTitle.toLowerCase()
  ];
  ...
  const addressesMisconceptions = conceptContext.misconceptions.some(misc => {
    const miscWords = misc.toLowerCase().split(' ');
    return miscWords.some(word => word.length > 4 && lowerResponse.includes(word));
  });
...
async function runValidationTests() {
  ...
  for (const testCard of TEST_CARDS) {
    ...
    for (const question of testCard.questions) {
      // Test Gemini
      const geminiResult = await testGemini(question, conceptContext, testCard);
      ...
      // Test DeepSeek
      const deepseekResult = await testDeepSeek(question, conceptContext, testCard);
