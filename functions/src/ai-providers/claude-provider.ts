...
export class ClaudeProvider extends BaseAIProvider {
  public readonly name = 'Claude Sonnet 4';
  public readonly modelId = 'claude-sonnet-4';

  private readonly baseUrl = 'https://api.anthropic.com/v1/messages';
  private readonly modelName = 'claude-sonnet-4-20250514';
  ...
  protected async generateQuestionInternal(
    ...
    logger.info(`🤖 [${this.name}] Sending request to Claude`, {
      requestId,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          throw this.createError('AUTHENTICATION_ERROR', 'Invalid Claude API key');
        }
        
        if (response.status === 429) {
          throw this.createError('RATE_LIMIT_ERROR', 'Claude API rate limit exceeded', undefined, true);
        }

        throw this.createError('NETWORK_ERROR', `Claude API error: ${response.status} ${errorText}`);
      }
      ...
      if (!text) {
        throw this.createError('VALIDATION_ERROR', 'Empty response from Claude');
      }
      ...
      throw this.createError('UNKNOWN_ERROR', `Claude request failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
...
- Provide sample answers with scoring rubric`;
  ...
  private parseResponse(text: string): any {
    try {
      ...
      throw this.createError('VALIDATION_ERROR', 'Invalid JSON response from Claude');
    }
  }
  ...
  public validateConfig(): boolean {
    ...
    if (!this.config.apiKey.startsWith('sk-ant-api03-')) {
      logger.error(`❌ [${this.name}] Invalid Claude API key format`);
      return false;
    }
